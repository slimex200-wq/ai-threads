...
def _check_rules(content: dict) -> list[str]:
    """규칙 기반 검증. 위반 사항 리스트 반환."""
    issues: list[str] = []

    # 필수 필드 존재
    required = ("post_main", "reply_explain", "reply_important",
    ...
    # 글자수 검증
    for key, (lo, hi) in _CHAR_LIMITS.items():
    ...
    # 금지 패턴
    for key in ("post_main", "reply_explain", "reply_important",
                "reply_action", "reply_counter", "reply_casual"):
        text = content.get(key, "")
    ...
    # post_main 마지막 줄이 질문으로 끝나는지
    post_main = content.get("post_main", "")
    if post_main and "?" not in post_main and "?" not in post_main:
...
def _evaluate_with_ai(content: dict) -> dict:
...
def evaluate(content: dict, *, skip_ai: bool = False) -> QAResult:
    """생성된 포스트를 규칙 + AI로 평가.
    ...
    """
    # 1단계: 규칙 기반 (무료, 즉시)
    rule_issues = _check_rules(content)
    ...
    # 2단계: AI 기반 (회의적 별도 호출)
    try:
        eval_result = _evaluate_with_ai(content)
