...
| ai_writer.py | Claude API 바이럴 포스트 생성 (성과 피드백 + 시간대 톤 + 주제 다양성) |
| threads_poster.py | Threads Graph API 포스팅 — 메인 + 5 대댓글 + 이미지 |
| threads_insights.py | Threads Insights API — 포스트 성과 지표 (좋아요/댓글/조회) 수집 |
| performance_db.py | 성과 DB 저장/조회 (output/performance.json) |
| collect_insights.py | 인사이트 수집 스크립트 (CI에서 main.py 전에 실행) |
...
## 자가학습 피드백 루프
```
포스팅 → post_id + 콘텐츠 → output/performance.json 저장
                                    ↓
다음 실행 시 collect_insights.py → Threads API로 좋아요/댓글/조회수 수집
                                    ↓
ai_writer.py 프롬프트에 주입:
  - 고성과 포스트 예시 (Top 3) → "이런 스타일로"
  - 저성과 포스트 예시 (Bottom 2) → "이런 스타일은 피해"
  - 최근 주제 목록 → 주제 다양성 강제
  - 시간대별 톤 → 아침(뉴스) / 오후(분석) / 저녁(의견)
```
- 성과 점수: `likes + (replies × 3) + (views × 0.01)` (댓글 3배 가중치)
- 인사이트 수집 조건: 포스팅 후 최소 6시간 경과
- 콜드 스타트: 성과 데이터 없으면 기존 프롬프트 그대로 동작
